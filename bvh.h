...
void buildBvh(char* base, float* matrix_base, struct Bvh* bvh, struct Hittable** objects, int num_objects){
    ...
    for(int i = 0; i < num_objects; i++){
        boxes[i] = HittableAABB(base, matrix_base, *(objects+i));
    ...
    for(int i = 0; i < num_objects; i++){
        areas[i+1] = HittableArea(base, matrix_base, *(objs+i), bvh->splitAxis)+areas[i];
    }
    ...
    buildBvh(base, matrix_base, bvh->left, objs, mid);
    bvh->right = (struct Bvh*)(malloc(sizeof(struct Bvh)));
    buildBvh(base, matrix_base, bvh->right, objs+(mid), (num_objects-mid));
